...
class MTVRPGenerator(Generator):
    ...
    def _generate(self, batch_size) -> TensorDict:
        ...
        num_depots = torch.full(
            size=(*batch_size, 1), fill_value=self.num_depots, dtype=torch.int32
        )
    ...
    def generate_distance_limit(
        ...
        # max_dist = torch.max(
        #     torch.cdist(locs[:, 0:1], locs[:, 1:]).squeeze(-2),
        #     dim=1,
        # )[
        #     0
    ...
    def available_variants(*args, **kwargs):
        ...
        # NOTE: there should be also the "MD" version
        # but for now, we keep it separate and simple only for fine-tuning
        return list(VARIANT_GENERATION_PRESETS.keys())[3:]


if __name__ == "__main__":
    gen = MTVRPGenerator(num_depot=3)
    data = gen._generate(batch_size=[3])
    gen.save_data(data, "multi_depot_instances_for_leon.npz")
